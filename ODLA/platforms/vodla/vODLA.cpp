...
odla_computation model_helper(const char* ccFile, const char* binFile)
{
    ...
    u64 time_used = (t_e.tv_sec - t_s.tv_sec) * 1000000 +
...
std::cout << "[vODLA] Debug: starting odla_adaptiveRelease.\n";
    ...
    time_used = (t_e.tv_sec - t_s.tv_sec) * 1000000 +
            (t_e.tv_usec - t_s.tv_usec);
    std::cout << "[vODLA] TIMING: VODH release resource: " << time_used << "us.\n";
#endif
...
odla_status odla_adaptiveAlloc(odla_device dev, const char* cfg, bool is_reaply)
{
// odla_adaptiveRelease(dev);
    ...
    ret = vodh_sa_res_apply(dev->vodh_hd, s.c_str(), key_size+cfg_size, is_reaply);
    if (ret) {
    ...
    time_used = (t_e.tv_sec - t_s.tv_sec) * 1000000 +
            (t_e.tv_usec - t_s.tv_usec);
    std::cout << "[vODLA] TIMING: VODH request resource: " << time_used << "us.\n";
#endif
...
odla_status odla_OpenDevice(odla_device dev)
{
    ...
    time_used = (t_e.tv_sec - t_s.tv_sec) * 1000000 +
            (t_e.tv_usec - t_s.tv_usec);
    std::cout << "[vODLA] TIMING: Query VODH device total cap: " << time_used << "us.\n";
    gettimeofday(&t_s, NULL);
#endif
    ...
    dev->vodh_dev_list = (vodh_dev*)malloc(sizeof(vodh_dev) * dev->vodh_total_cap.vxpu_num);
    if(dev->vodh_dev_list == NULL){
        std::cout << "[vODLA] ERROR: Failed create device list.\n";
        freeAllocs(dev, ERR_DEV_LIST);
        return ODLA_FAILURE;
    }else{
        memset(dev->vodh_dev_list, 0, sizeof(vodh_dev) * dev->vodh_total_cap.vxpu_num);
        dev->vodh_dev_cap_list = (vodh_dev_cap*)malloc(sizeof(vodh_dev_cap)
                                * dev->vodh_total_cap.vxpu_num);
    ...
    time_used = (t_e.tv_sec - t_s.tv_sec) * 1000000 +
            (t_e.tv_usec - t_s.tv_usec);
    std::cout << "[vODLA] TIMING: Get VODH devices info: " << time_used << "us.\n";
    gettimeofday(&t_s, NULL);
#endif

    for(unsigned i = 0; i < dev->vodh_total_cap.vxpu_num; i++) {
        /* Query device capability */
        ret = vodh_get_one_dev_cap(dev->vodh_hd,
                        &(dev->vodh_dev_list[i]), &(dev->vodh_dev_cap_list[i]));
        if (ret) {
        ...
        //std::cout << "[vODLA] compute: " << vodh_dev_cap_list[i].comput << "eFLOPS\n";
#endif
        /*
        if ((vodh_dev_cap.compute < COMPUTE_NEED) ||
            (vodh_dev_cap.memory < (INPUT_DATA_SIZE + g_out_size))) {
            std::cout << "[vODLA] Warning: device capability does not match requirements.\n";
            std::cout << "[vODLA] Warning: Required: compute " << COMPUTE_NEED << "FLOPS, memory "
    ...
    time_used = (t_e.tv_sec - t_s.tv_sec) * 1000000 +
            (t_e.tv_usec - t_s.tv_usec);
    std::cout << "[vODLA] TIMING: Query each VODH device cap: " << time_used << "us.\n";
    gettimeofday(&t_s, NULL);
#endif
...
odla_status odla_AllocateDevice(const odla_vendor vendor, const odla_device_name device_name,
        odla_device* device, const char* config)
{
    ...
    if (config != NULL) {
        g_config = config;
        if (odla_adaptiveAlloc(dev,config,0) == ODLA_FAILURE) {
...
odla_status odla_DestroyDevice(odla_device device)
{
    ...
    // vodh_ret vodh_sa_res_release(device->vodh_hd, const char *releaseinfo, u32 bufflen);
    if (device->need_release)
        odla_adaptiveRelease(device);
...
#ifdef DEBUG
        std::cout << "[vODLA] DEBUG: input " << idx << " address changed to " << data_ptr << "\n";
#endif
    }
    context->ipNum++;
    return ODLA_SUCCESS;
                              ...
                              odla_context context)
{
    ...
    if(context->opPtr[idx] != data_ptr){
        ...
        context->dataChg = true;
#ifdef DEBUG
        std::cout << "[vODLA] DEBUG: output " << idx << " address changed to " << data_ptr << "\n";
#endif
    }
    context->opNum++;
    return ODLA_SUCCESS;
...
odla_status odla_DestroyComputation(odla_computation comp)
{
    ...
    if(g_dev == NULL){
        ...
        if (g_dev->vodh_hd == NULL || g_dev->vodh_dev_list == NULL) {
            std::cout << "[vODLA] Error: device is not initiated before model switch.\n";
        }
        // else{
...
void deallocDMA(odla_device device, bool ip, bool op)
{
    ...
    if (device->vodh_infer_result.output && op) {
        for(u32 i=0; i<device->vodh_infer_result.output_num; i++){
            #ifdef DEBUG
            printf("output %d data at %p\n", i, device->vodh_infer_result.output[i]->data);
            #endif
...
bool allocDMA(odla_device device, odla_context context)
{
        ...
        if (device->vodh_infer_opt.input) {
            for(u32 i=0; i<context->ipNum; i++){
                ...
                if (device->vodh_infer_opt.input[i]) {
                    device->vodh_infer_opt.input[i]->data = vodh_malloc(context->ipSizes[i]);
                    if (device->vodh_infer_opt.input[i]->data && context->ipPtr[i]) {
                        #ifdef TIMING
                            gettimeofday(&t_s, NULL);
                        #endif
                        memcpy(device->vodh_infer_opt.input[i]->data, context->ipPtr[i], context->ipSizes[i]);
                        #ifdef TIMING
                            gettimeofday(&t_e, NULL);
                            time_used = (t_e.tv_sec - t_s.tv_sec) * 1000000 +
                                    (t_e.tv_usec - t_s.tv_usec);
                            std::cout << "[vODLA] TIMING: CP DMA data for input " << i << ": " << time_used << "us. (new)\n";
                        #endif
                    }
                    else {
                        std::cout << "[vODLA] ERROR: allocate infer input " << i << " data failed.\n";
                        ret = false;
                    }
                }
                else {
                    std::cout << "[vODLA] ERROR: allocate infer input: " << i << " failed.\n";
                    ret = false;
                }
                ...
                if (device->vodh_infer_opt.input[i]) {
                    ...
                    if (device->vodh_infer_opt.input[i]->data && context->ipPtr[i]) {
                        ...
                        memcpy(device->vodh_infer_opt.input[i]->data, context->ipPtr[i], context->ipSizes[i]);
                            ...
                            time_used = (t_e.tv_sec - t_s.tv_sec) * 1000000 +
                                    (t_e.tv_usec - t_s.tv_usec);
                            std::cout << "[vODLA] TIMING: CP DMA data for input " << i << ": " << time_used << "us. (reuse)\n";
                        #endif
                    }
                    else {
                        std::cout << "[vODLA] ERROR: copy infer input " << i << " data failed.\n";
                        ret = false;
                    }
                }
                else {
                    std::cout << "[vODLA] ERROR: reuse infer input: " << i << " failed.\n";
                    ret = false;
                }
                device->vodh_infer_opt.input[i]->size = context->ipSizes[i];
                ip_mem_cap += device->vodh_infer_opt.input[i]->size;
    ...
    if (device->vodh_infer_result.output_num != context->opNum) {
        ...
        if (device->vodh_infer_result.output) {
            for(u32 i=0; i<context->opNum; i++) {
                ...
                if (device->vodh_infer_result.output[i]) {
                    device->vodh_infer_result.output[i]->data = vodh_malloc(context->opSizes[i]);
                    if (device->vodh_infer_result.output[i]->data == NULL) {
                        std::cout << "[vODLA] ERROR: allocate infer output " << i << " data failed.\n";
                        ret = false;
                    }
                }
                else {
                    std::cout << "[vODLA] ERROR: allocate infer output: " << i << " failed.\n";
                    ret = false;
                }
        ...
        if (device->vodh_infer_result.output) {
            for(u32 i=0; i<context->opNum; i++) {
                if (device->vodh_infer_result.output[i]) {
                    if (device->vodh_infer_result.output[i]->size != context->opSizes[i]) {
                        vodh_free(device->vodh_infer_result.output[i]->data);
                        device->vodh_infer_result.output[i]->data = NULL;
                        device->vodh_infer_result.output[i]->data = vodh_malloc(context->opSizes[i]);
                    }
                    if (device->vodh_infer_result.output[i]->data == NULL) {
                        std::cout << "[vODLA] ERROR: allocate infer output " << i << " data failed (reuse).\n";
                        ret = false;
                    }
                }
                else {
                    std::cout << "[vODLA] ERROR: reuse infer output: " << i << " failed.\n";
                    ret = false;
                }
                device->vodh_infer_result.output[i]->size = context->opSizes[i];
                op_mem_cap += device->vodh_infer_result.output[i]->size;
...
static void* odla_InferThread(void* args) {
	printf("线程 infer_thread ID 0x%x 进入。\n",(unsigned int)pthread_self());
	odla_device device = (odla_device) args;
	
	vodh_infer(device->vodh_hd, &(device->vodh_dev_list[0]),
                        &(device->vodh_infer_opt), &(device->vodh_infer_result));

	printf("线程 infer_thread ID 0x%x 退出。\n",(unsigned int)pthread_self());
	pthread_exit((void *)0);
}

static int odla_GetThreadStatus(pthread_t tid) {
...
static int odla_InferSend(odla_device device) {
	pthread_t tid_infer;
	pthread_create(&tid_infer, NULL, odla_InferThread, (void*)device);
    pthread_detach(tid_infer);
	const char *filename="/var/run/vodla_http_rpc.sock";
	struct sockaddr_un un;
	int sock_fd;
	int buffer_size = 1024;
	// char buffer[BUFFER_SIZE] = "health";
	char buffer[buffer_size] = "{\n  \"jsonrpc\": \"2.0\",\n  \"method\": \"get_vodla_target_info\",\n  \"id\": 1,\n  \"params\": {\n    \"name\": \"mingfa\"\n  }\n}";
	char buffer_rec[buffer_size] = {0};
	...
	// void* ret;
	bool is_infer_failed = false;
	while (1) {
		usleep(1000);
		int tid_infer_state = odla_GetThreadStatus(tid_infer);
		if (tid_infer_state == 1) {
			// close(sock_fd);
			break;
		}
		else {
			...
			// printf("the mesage send is : %s\n", buffer);
			recv(sock_fd,buffer_rec,buffer_size,0);
			// printf("the mesage recv is : %s\n", buffer_rec);
			cJSON *root = NULL;
			cJSON *temp = NULL;
			root = cJSON_Parse(buffer_rec);
			if (!root) {
				printf("parse health json failed \n");
			}
			temp = cJSON_GetObjectItem(root, "health_info");
			if (!temp) {
				printf("parse health json failed \n");
			}
			char *ptr = cJSON_Print(temp);
			char *health_state_ptr = (char*)malloc(strlen(ptr));
			memset(health_state_ptr, 0, strlen(ptr));
			strncpy(health_state_ptr, ptr + sizeof(char), strlen(ptr) - 2);
			// printf("health_state_ptr = %s \n",health_state_ptr);
			if (strcmp(health_state_ptr, "running")) {
				printf("the backend container health_state is not running, health_state is = %s \n", health_state_ptr);
				free(health_state_ptr);
				free(ptr);
				is_infer_failed = true;
				int cancel_res = pthread_cancel(tid_infer);
				printf("cancel tid_infer, the cancel_res = %d\n", cancel_res);
				break;
			}
			free(ptr);
			free(health_state_ptr);
	...
	close(sock_fd);
	printf("start pthread_join tid_infer\n");
	// pthread_join(tid_infer, NULL);
	printf("finish pthread_join tid_infer\n");
	if (is_infer_failed)
        return -1;
		// send_input_info->ret = VODH_EHEALTH;
	return 0;
}
                                    ...
                                    odla_device device)
{
    ...
    if (device == NULL || comp == NULL ||
        device->vodh_dev_list == NULL ||
        device->vodh_dev_cap_list == NULL) {

        if (device->vodh_dev_list == NULL ||
            device->vodh_dev_cap_list == NULL) {
            std::cout << "[vODLA] ERROR: allocate device failed, skip executing compution.\n";
        }
        else {
    ...
    if (allocSuccess) {
...
#ifdef DEBUG
        std::cout << "[vODLA] DEBUG: infer request ID is " << device->vodh_infer_result.request_id << "\n";
#endif
        ...
        device->vodh_infer_opt.request_cap.net_delay = device->vodh_total_cap.net_delay;
        device->vodh_infer_opt.request_cap.use_direct_rdma = true;
        device->vodh_infer_opt.request_cap.compute = 0; //TODO: compute power required from cost model.
        ...
        for(u32 lp = 0; lp < LOOP_CNT; lp++){
            ...
            //             &(device->vodh_infer_opt), &(device->vodh_infer_result));
            // if (odla_adaptiveAlloc(device,g_config,1) == ODLA_FAILURE) {
            //     std::cout << "[vODLA] ERROR: Failed to reuqest reources adaptively.\n";
            //     return ODLA_FAILURE;
            // }
            // std::cout << "[vODLA] INFO: succeed to reuqest reources adaptively.\n";
            rt = odla_InferSend(device);
            if(rt){
                break;
            }
            ...
            xpu_time_max = std::max(xpu_time_max, device->vodh_infer_result.time_used);
            xpu_time_min = std::min(xpu_time_min, device->vodh_infer_result.time_used);
            xpu_time_avg += device->vodh_infer_result.time_used;
        }
        xpu_time_avg /= LOOP_CNT;
        std::cout << "[vODLA] TIMING: Remote xPU inference avg time: "
              << xpu_time_avg << "us, max time: " << xpu_time_max << "us, min time: "
              << xpu_time_min << "us.\n";
        time_avg /= LOOP_CNT;
        std::cout << "[vODLA] TIMING: Inference avg time: "
            << time_avg << "us, " << "max time: " << time_max << " us,"
            << "min time: " << time_min << " us.\n";
        std::cout << "[vODLA] TIMING: Inference avg throuput: "
            << context->batchSize/time_avg*1000000 << " imgs/s, " << "max throuput: " << context->batchSize/time_min*1000000 << " imgs/s,"
            << "min throuput: " << context->batchSize/time_max*1000000 << " imgs/s.\n";
#else
        /* Remote infer */
        // rt = vodh_infer(device->vodh_hd, &(device->vodh_dev_list[0]),
        //             &(device->vodh_infer_opt), &(device->vodh_infer_result));
        ...
        if (rt) {
            ...
            std::cout << "[vODLA] INFO: reapply key, restart infer\n";
            if (odla_InferSend(device)) {
                std::cout << "[vODLA] ERROR: restart infer failed\n";
                ret = ODLA_FAILURE;
            }
            else
                std::cout << "[vODLA] INFO: restart infer succeed\n";
        }
        else {
#ifdef DEBUG
        ...
        for(u32 i=0; i<context->opNum; i++) {
            if (context->opPtr[i]) {
                memcpy(context->opPtr[i], device->vodh_infer_result.output[i]->data, context->opSizes[i]);
            }
            else {
